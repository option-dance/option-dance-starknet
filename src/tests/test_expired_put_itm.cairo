...
use optiondance::tests::test_expired_put_otm::{setup, setup_put_option};
...
fn test_settle_and_exercise_put_itm() {
    ...
    assert(price == 2000000000000, 'price == 0');
    assert(enabled == true, 'enabled');

    // settle
    controller.settle(writertoken.contract_address, 3000000000000);
    assert(usdc.balance_of(account1) == 20000000000, 'usdc after balance invalid');
    assert(
        usdc.balance_of(controller.contract_address) == 10000000000, 'usdc after balance invalid'
    );
    ...
    assert(usdc.balance_of(account2) == 10000000000, 'usdc after balance invalid');
